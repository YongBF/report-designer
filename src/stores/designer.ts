...
export function selectComponent(id: string | string[], addToSelection = false) {
  if (Array.isArray(id)) {
    selectedIds.value = addToSelection
...
export function removeDataSource(id: string) {
  currentDesign.value.dataSources = currentDesign.value.dataSources.filter(
    (d) => d.id !== id
  )
  saveHistory('删除数据源')
}
...
export function removeDatabaseConnection(id: string) {
  currentDesign.value.databaseConnections =
    currentDesign.value.databaseConnections.filter((c) => c.id !== id)
  saveHistory('删除数据库连接')
}
...
export const selectedComponents = computed(() => {
  return currentDesign.value.components.filter((c) =>
