<template>
  <el-form label-width="100px" size="small">
    ...
    <!-- 散点图特有配置 -->
    <el-divider content-position="left">散点图配置</el-divider>
    ...
    <el-form-item label="标记形状">
      <el-select v-model="localComponent.symbol" @change="handleChange">
        <el-option label="圆形" value="circle" />
        <el-option label="矩形" value="rect" />
        <el-option label="三角形" value="triangle" />
        <el-option label="菱形" value="diamond" />
        <el-option label="大头针" value="pin" />
        <el-option label="箭头" value="arrow" />
      </el-select>
    </el-form-item>

    <el-form-item label="显示特效">
      <el-switch v-model="localComponent.showEffect" @change="handleChange" />
    </el-form-item>

    <el-form-item label="特效类型">
      <el-select v-model="localComponent.effectType" @change="handleChange">
        <el-option label="涟漪" value="ripple" />
...
const localComponent = reactive<ScatterChartComponent>({
  ...props.component,
...
watch(
  ...
  (newComponent) => {
    ...
    localComponent.symbol = newComponent.symbol;
    localComponent.showEffect = newComponent.showEffect;
    localComponent.effectType = newComponent.effectType;
...
function handleChange() {
  ...
  updateComponent(props.component.id, {
    ...
    symbol: localComponent.symbol,
    showEffect: localComponent.showEffect,
    effectType: localComponent.effectType,
  });
  emit('update');
}
</script>
