...
function getChartData() {
  ...
  console.log('[DEBUG] getChartData - dataSource:', dataSource);
  ...
  if (dataSource?.staticData) {
    ...
    console.log('[DEBUG] getChartData - staticData:', staticData);
    if (staticData.categories && staticData.categories.length > 0 &&
        staticData.series && staticData.series.length > 0) {
      console.log('[DEBUG] getChartData - Returning staticData with', staticData.series.length, 'series');
      return staticData;
    }
  }

  console.log('[DEBUG] getChartData - Using default data');
  // 默认模拟数据
  return {
